...
@section('title', 'Reference Books - Soma Connect')
...
@section('content')
                ...
                <div class="col-10">
                    ...
                    <div class="portal-header mb-4">
                        <div class="d-flex align-items-center justify-content-between flex-wrap">
                            <div class="d-flex align-items-center">
                                <div class="portal-logo me-3" style="background: linear-gradient(135deg, #38b2ac 0%, #319795 100%);">
                                    <i class="fas fa-book fa-2x text-white"></i>
                                </div>
                                <div>
                                    <h4 class="text-black mb-0 fw-bold">Reference Books</h4>
                                    <p class="text-black-50 mb-0 small">Supplementary reading materials for deeper understanding</p>
                                </div>
                            </div>
                            <div>
                                <span class="portal-badge text-black">
                                    <i class="fas fa-database me-2"></i>
                                    {{ $referenceBooks->count() }} Books Available
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="search-section mb-4">
                        <div class="search-wrapper">
                            ...
                            <input type="text" class="search-input-large" placeholder="Search reference books by title, author or subject..." id="searchInput">
                        </div>
                    </div>
                    ...
                    <div class="filter-section mb-4">
                        <div class="d-flex flex-wrap align-items-center gap-3">
                            ...
                            <div class="filter-buttons">
                                ...
                                @foreach($referenceBooks->pluck('subject.name')->unique()->filter() as $subject)
                        ...
                        <div class="row g-4">
                            @foreach($referenceBooks as $book)
                                <div class="col-lg-3 col-md-4 col-sm-6 book-item"
                                     data-subject="{{ $book->subject->name ?? 'Uncategorized' }}"
                                     data-title="{{ $book->title }}"
                                     data-author="{{ $book->author }}">
                                    <div class="material-card">
                                        <div class="material-image-wrapper">
                                            <a href="{{ route('notes.show', encrypt($book->id)) }}" class="material-link">
                                            ...
                                            @if($book->subject)
                                                <span class="subject-badge">{{ $book->subject->name }}</span>
                                            @endif
                                            <span class="type-badge referencebook">
                                                Reference Book
                                            </span>
                                        </div>
                                        <div class="material-content">
                                            <h5 class="material-category">{{ $book->subject->name ?? 'General' }}</h5>
                                            <h3 class="material-title">
                                                <a href="{{ route('notes.show', encrypt($book->id)) }}">{{ Str::limit($book->title, 40) }}</a>
                                            </h3>
                                            <div class="material-meta">
                                                ...
                                                <span class="material-grade">
                                                    <i class="fas fa-calendar-alt me-1"></i>{{ $book->created_at->format('Y') }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    @else
                        <div class="empty-state">
                            ...
                            <h5>No reference books available</h5>
                            <p class="text-muted">Check back later for new reading materials</p>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <style>
        ...
        /* Material Cards - Matching materials page */
        .material-card {
